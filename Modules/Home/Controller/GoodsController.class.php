...
class GoodsController extends CommonController {
	...
	**/
	...
	//从小游戏转入优惠券
	public function getQuanfromGame()
	{
	    $gpc = I('request.');
	    //
	    $unionid = $gpc['unionid'];
	    //oOlUFwxgwHPpvybJwqVMQT6itnmA
	    $quan_title   = $gpc['title'];
	    $checkcode   = $gpc['checkcode'];
	    //1进行校验码验证
	    $time = date("Y-m-d");
	    $code = "asdfRTYUhjkl";
	    //经过md5加密
	    $createcode = md5($code.$time);
	    //
	    $res_code = 0;
	    if($checkcode == $createcode)
	    {
	        //说明校验码正确
	        //2根据用户unionid查找member_id,
	        $member_info = M('lionfish_comshop_member')->where( array('unionid' => $unionid) )->find();
	        if(!empty($member_info) )
			{
			    //3根据优惠券的名称查找ID
			    $voucher_info =  M('lionfish_comshop_coupon')->where( array('voucher_title' => $quan_title) )->find();
			    if(!empty($voucher_info))
			    {
			        //D('Admin/Member')->sendMemberPointChange($member_info['member_id'],$score, 0 , '小游戏转入积分','from_game');
			        $res_code =   D('Home/Voucher')->send_user_voucher_byId2($voucher_info['id'],$member_info['member_id'],false);
			        //1 被抢光了 2 已领过  3  领取成功
			    }
			    else
			    {
	    ...
	    //返回结果说明：-1，程序校验码不正确，1 被抢光了 2 已领过  3  领取成功,4:转入的优惠券商城不存在,5：用户在商城未注册
	    //echo json_encode( array('code' => $res_code, 'unionid'=>$unionid,'score'=>$score,'checkcode'=>$checkcode,'createcode' => $createcode) );
	    echo json_encode( array('code' => $res_code) );
		die();
	}
    ...
    public function get_seller_noindex_quan()
    {
        $_GPC = I('request.');
        ...
        $where = " and (total_count =-1 or total_count>send_count) and is_index_alert =0   and (end_time>".time()." or timelimit =1 ) ";
	...
	public function get_seller_quan()
	{
        ...
        $member_id = $weprogram_token['member_id'];
        $where = "";
        if(isset($_GPC['coupon_id'])){
            $coupon_id = $_GPC['coupon_id'];
            $coupon = M('lionfish_comshop_poster')->where( array('id' => $coupon_id) )->field('coupon_id')->find();

            if(empty($coupon)){
                $coupon['coupon_id'] = 1;
            }
            $where = " and (total_count =-1 or total_count>send_count) and is_index_alert =0  and (end_time>".time()." or timelimit =0 ) ";

            $quan_list = M()->query("select * from ".C('DB_PREFIX')."lionfish_comshop_coupon where id IN ({$coupon['coupon_id']}) {$where} order by displayorder desc ,id asc limit 10 ");
        }else{
	...
	**/
	...
	public function load_buy_recordlist()
	{
		...
		$list = D('Home/Frontorder')->get_goods_buy_record($goods_id,9999);
			...
			if( !empty($is_community_hd) )
			{
				...
				if( $is_commu_sale )
				{
					...
					{	
						...
						if($goods_head_money > $max_comunity_money){
                            $goods_head_money = $max_comunity_money;
                        }
