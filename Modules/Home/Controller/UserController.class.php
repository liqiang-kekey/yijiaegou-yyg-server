...
class UserController extends CommonController {
    ...
    public function lucky(){
        ...
        if(empty($category_id)){
            echo json_encode( array('code' => 1,'msg'=>'参数非法') );
            die();
        }
        $memberconfig = D('Seller/Config')->get_all_config();
        $member_configs = [
            'lottery_begin'=>$memberconfig['lottery_begin'],
            'lottery_score'=>$memberconfig['lottery_score'],
            'lottery_num'   =>$memberconfig['lottery_num'],
        ];
        $lottery_category =  M('lionfish_comshop_lottery_category')->where(array('id'=>$category_id))->find();
        if(!$lottery_category){
            echo json_encode( array('code' =>1,'msg'=>'此活动已结束') );die();
        }
        //153   154
        if($lottery_category['start_time'] > time()){
            echo json_encode( array('code' =>1,'msg'=>'此活动还未开始') );die();
        }
        if($lottery_category['end_time']<time()){
            echo json_encode( array('code' =>1,'msg'=>'此活动已结束') );die();
        }
//        if(!$member_configs['lottery_begin']){
//            $result['code'] = 3;
//            $result['msg'] = '活动暂未开启';
//            echo json_encode($result);
//            die();
//        }
        $list = M('lionfish_comshop_lottery')->where(array('category_id'=>$category_id))->field('id,name,luck,img')->select();
        ...
        $res['name'] = $list[$awardIndex]['name']; //中奖项
        $res['img'] = $list[$awardIndex]['img']; //中奖项图片
        $res['awardIndex'] = $awardIndex;//中奖项索引
        ...
        if($member_info['score'] < $member_configs['lottery_score']){
            $result['code'] = 3;
            $result['msg'] = '积分不足';
            echo json_encode($result);
            die();
        ...
        $member_lottery_count = M('lionfish_comshop_lottery_list')->where($condition)->count();
        if($member_lottery_count >= $member_configs['lottery_num']){
            $result['code'] = 3;
            $result['msg'] = '今日次数已用完';
            echo json_encode($result);
            die();
        ...
        //减少
        M()->startTrans();
        $decScore =  M('lionfish_comshop_member')->where( array('member_id' => $member_id) )->setDec('score',(int)$member_configs['lottery_score']);
        if(!$decScore){
            M()->rollback();
            $result['code'] = 3;
            $result['msg'] = '1网络错误,稍后再试';
            echo json_encode($result);
            die();
        }
        $data = [
            ...
            'lottery_name'=>$res['name'],
            'lottery_img'=>$res['img'],
            'score'     =>$member_configs['lottery_score'],
            'create_time'   =>time(),
            'category_id'=>$category_id,
        ];
        $lottery_list = M('lionfish_comshop_lottery_list')->add($data);
        if($lottery_list){
            M()->commit();
    ...
    public function lotteryList(){
        ...
        $lottery_category =  M('lionfish_comshop_lottery_category')->where(array('id'=>$id))->find();
        if(!$lottery_category){
            echo json_encode( array('code' =>1,'msg'=>'此活动已结束') );die();
        }
        //153   154
        ...
        $list['config'] = [
            'lottery_begin'=>$data['lottery_begin'],
            'lottery_score'=>$data['lottery_score'],
            'lottery_num'   =>$data['lottery_num'],
            'lottery_charge_publish'=>$data['lottery_charge_publish']
        ];
        $condition = ' 1 ';
        $starttime = isset($_GPC['time']['start']) ? strtotime($_GPC['time']['start']) : strtotime(date('Y-m-d'.' 00:00:00'));
        $endtime = isset($_GPC['time']['end']) ? strtotime($_GPC['time']['end']) : strtotime(date('Y-m-d'.' 23:59:59'));
        ...
        $condition.= ' AND member_id= '.$member_id;
        $member_lottery_count = M('lionfish_comshop_lottery_list')->where($condition)->count();
        $list['lottery_num'] = $data['lottery_num']-$member_lottery_count;

        $list['score'] = (int)$member_info['score'];
        // $list['lottery'] = M('lionfish_comshop_lottery')->field('id,name,luck,img')->where('category_id',$id)->select();
        $lottery =  M('lionfish_comshop_lottery')->field('id,name,luck,img')->where(array('category_id'=>$id))->select();
        if(!$lottery){
            echo json_encode( array('code' =>1,'msg'=>'此活动已结束') );die();
        }
        ...
        if( empty($list) )
        {
            echo json_encode( array('code' =>1,'msg'=>'暂无奖品') );die();
        }else {
            echo json_encode( array('code' =>0, 'list' => $list,'category_id'=>$id) );die();
        }
    }
...
//		M('goods')->field('image')->where( array('goods_id' => ) )->find();
    ...
    public function get_user_info()
    {
        ...
        if($member_id){
            ...
            //  community/moduleA/groupCenter/pendingDeliveryOrders?memberId=49

            if( empty($member_info['hexiao_qrcod']))
            {
                $path = "community/moduleA/groupCenter/pendingDeliveryOrders";
    ...
    // 小程序解密
    public function decryptDataUnionid($encryptedData, $iv, $token,$session_key)
    {
        $appid_info = M('lionfish_comshop_config')->where( array('name' => 'wepro_appid') )->find();
        $aesKey=base64_decode($session_key);
        $aesIV=base64_decode($iv);
    ...
    public function applogin_do()
    {
        ...
        $token = $gpc['token'];
        $res = $this->decryptDataUnionid($encryptedData, $iv, $token,$session_key);
        ...
        if(!empty($member_info) )
        {
            ...
            if(!empty($unionid) && empty($member_info['unionid'])){
            ...
            if( !empty($old_token_info) )
            {
                $carData = array();
                $carData['token'] = $token;
                M('lionfish_comshop_car')->where( array('token' => $old_token_info['token'] ) )->save($carData);
            }
            ...
            $file  = 'share_logs.txt';//要写入文件的文件名（可以是任意文件名），如果文件不存在，将会创建一个
            file_put_contents($file, $share_id.PHP_EOL,FILE_APPEND);
            if( $share_id > 0 )
            {
                $commiss_level = D('Home/Front')->get_config_by_name('commiss_level');
                if( !empty($commiss_level) && $commiss_level > 0)
                {
                    ...
                    $file  = 'share_member_info.txt';//要写入文件的文件名（可以是任意文件名），如果文件不存在，将会创建一个
                    file_put_contents($file, $share_member_info.PHP_EOL,FILE_APPEND);
                    if( !empty($share_member_info) )
                    {
                        if(  $share_member_info['comsiss_flag'] == 1  && $share_member_info['comsiss_state'] == 1 )
                        {
                            M('lionfish_comshop_member')->where( array('member_id' => $member_id ) )->save( array('agentid' => $share_id ,'share_id'=>intval($share_id)) );
                        }
                    }
    ...
    public function update_user_infos(){
        ...
        $param['telephone'] = $_GPC['mobile'];
        if(!empty($_GPC['birth'])){
            $param['birth'] = $_GPC['birth'];
        }
        if($member_id && !empty($param['realname']) && !empty($param['telephone']) ){
