...
class CarController extends CommonController {
	...
	**/
	...
	public function add()
	{
		...
		$goods_description = D('Home/Front')->get_goods_common_field($goods_id , 'total_limit_count,one_limit_count,oneday_limit_count,is_new_buy,is_limit_levelunbuy,is_limit_vipmember_buy');
		...
		if(  $goods_description['is_limit_levelunbuy'] == 1 )
		{
			...
			{
				$json['code'] =6;
				$json['msg']='默认等级不能购买，订购套餐即可提升等级';//订购套餐即可提升等级//请联系客服
				echo json_encode($json);
				die();
			}
		}	
		...
		if($product){
			 ...
			 * 查询限购配置数据
			 */
			$homeFront = D('Home/Front');
            $goods_desc = $homeFront->get_goods_common_field($goods_id, 'one_limit_count,oneday_limit_count,total_limit_count');
			...
			if ($goods_desc['total_limit_count'] > 0 || $goods_desc['one_limit_count'] > 0)
			{
			    ...
			    /**
			     * 20201121-LQ-UPDATE
			     * $can_buy_count==-1，说明，已经购买次数达到了历史限购次数
			     * $can_buy_count== 0，说明，不限购
			     * $can_buy_count > 0，剩余可购买次数
			     *                    小于one_limit_count，说明已开启历史限购，剩余可购买次数已小于单次限购次数
			     *                    等于one_limit_count，说明历史限购是否开启不确定
			     *                    大于one_limit_count，说明历史限购已开启
			     */
			    $can_buy_count = $homeFront->check_goods_user_canbuy_count($member_id, $goods_id);
			    ...
			    {
			        $json['code'] =6;
			        $json['msg']='限购商品，您已购买'.$goods_description['total_limit_count'].'个'.'，不能再购买。';
			        $json['max_quantity'] = 1;
			        echo json_encode($json);
			        die();
			    }
			    ...
			    if(!empty($cart_goods_all_quantity) && $cart_goods_all_quantity > 0 && $data['buy_type'] != 'pintuan' )
			    {
			         ...
			         * 判断购物车商品是否已经达到可购买数量
			         */
			        if ($can_buy_count > 0 && $cart_goods_all_quantity >= $can_buy_count)
			        {
			            $json['code'] =6;
			            $json['msg']='限购商品，购物车已有'.$cart_goods_all_quantity.'个商品，请前往结算。';
			            $json['max_quantity'] = $cart_goods_all_quantity;
			            echo json_encode($json);
			            die();
			        }
			        	        
			        /**
			         * 判断购物车+本次数量是否满足还能购买数量
			         */
			        if ($can_buy_count > 0 && $quantity + $cart_goods_all_quantity > $can_buy_count) 
			        {
		                $json['code'] =6;
		                $json['msg']='限购商品，购物车已有' . $cart_goods_all_quantity . '个商品，您还能购买'. ($can_buy_count - $cart_goods_all_quantity) .'个。';
		                $json['max_quantity'] = $can_buy_count - $cart_goods_all_quantity;
		                echo json_encode($json);
		                die();
			        }
			    }
			    ...
			    {
			        $json['code'] =6;
			        $json['msg']='限购商品，您还能购买'.$can_buy_count.'个。';
			        $json['max_quantity'] = $can_buy_count;
			        echo json_encode($json);
			        die();
			    }
			...
			if ($goods_desc['oneday_limit_count'] > 0) 
			{
			    /**
			     * 20201121-LQ-ADD
			     * $can_buy_day_count== 0，说明，每天限购未限制
			     * $can_buy_day_count==-1，说明，每天限购次数已经用完，达到每天限购阈值
			     * $can_buy_day_count > 0，说明，每天限购还有剩余次数（$can_buy_day_count）
			     */
			    $can_buy_day_count = $homeFront->check_goods_user_canbuy_day_count($member_id, $goods_id);
			    if(-1 == $can_buy_day_count)
			    {//已经达到每天限购阈值
			        $json['code'] =6;
			        $json['msg']='限购商品，您今天已购买了'.$goods_description['oneday_limit_count'].'个'.'，请明天再来购买。';
			        $json['max_quantity'] = 1;
			        echo json_encode($json);
			        die();
			    }
			    ...
			    { 
			         ...
			         * 判断购物车数量不能大于还能购买数量
			         */
			        if(0 < $can_buy_day_count && $cart_goods_all_quantity >= $can_buy_day_count){
			            $json['code'] =6;
			            $json['msg'] = '限购商品，购物车已有' . $cart_goods_all_quantity . '个商品，请前往结算。';
			            $json['max_quantity'] = $cart_goods_all_quantity;
			            echo json_encode($json);
			        ...
			        /**
			         * 判断本次购买数量+购物车数量不能大于还能购买数量
			         */
			        if(0 < $can_buy_day_count && $quantity + $cart_goods_all_quantity > $can_buy_day_count){
			            $json['code'] =6;
			            $json['msg'] = '限购商品，购物车已有' . $cart_goods_all_quantity . '个商品，您今天还能购买' . ($can_buy_day_count - $cart_goods_all_quantity) . '个。';
			    ...
			    /**
			     * 判断本次购买数量是否已经大于还能购买数量
			     */
			    if(0 < $can_buy_day_count && $quantity > $can_buy_day_count)
			    {
			        $json['code'] =6;
			        $json['msg']='限购商品，您今天还能购买'.$can_buy_day_count.'个。';
			        $json['max_quantity'] = $can_buy_day_count;
			        echo json_encode($json);
			        die();
			    }
			}
			...
			/**
			 * 20201120-LQ-UPDATE
			 * 已加入购物车的商品和本次购买的总数量，不能大于商品剩余库存数量
			 */		
			...
			if ($goods_quantity < $quantity + $cart_goods_quantity)
			{
                ...
                if ($goods_quantity == 0) {
			        ...
			        $json['msg']='商品数量不足，本次只能购买'.$goods_quantity.'个。';
					$json['max_quantity'] = $goods_quantity;
			    }
	...
	private function _check_can_buy($member_id, $goods_id, $quantity)
	{
        ...
        if($can_buy_day_count >0 && $quantity >$can_buy_day_count)
        {
            $json['code'] = 6;
            $json['msg'] = '限购商品，您今天还能购买' . $can_buy_day_count . '份。';
		...
		if($can_buy_count == -1 || $can_buy_day_count == -1)
		{
		    /**
		     * 20201120-LQ-
		     * 这块逻辑没有看懂，需要继续分析，按照正常逻辑历史限购或者每天限购已经触发阈值，那么就不能再购买商品，怎么还提示“您本次只能购买xx个”呢？
		     */
			$json['code'] =6;
		...
		} else if ($can_buy_count > 0 && $quantity > $can_buy_count)
		{
			...
			$json['msg']='限购商品，您还能购买'.$can_buy_count.'份。';
	...
	//	$country_info = M('lionfish_comshop_area')->where( "name like '%{$countyName}%' " )->find();
		 $country_info = M('lionfish_comshop_area')->where(array('pid'=>$city_id,'name'=>$countyName))->find();
		if( $countyName == '中山' )
		{
	...
	public function checkout()
	{
	  ...
	  $mb_counties_name = isset($gpc['mb_counties_name']) ? $gpc['mb_counties_name'] : '';
	...
	{
		$city_pid = M('lionfish_comshop_area')->where(" name='{$mb_city_name}' ")->find();
       // $mb_city_info = M('lionfish_comshop_area')->where(" pid='{$city_pid['id']}' ")->where(" name='{$mb_counties_name}' ")->find();
        $mb_city_info = M('lionfish_comshop_area')->where(array('pid'=>$city_pid['id'],'name'=>$mb_counties_name))->find();
    ...
    $need_data = array();
	$need_data['new_region'] = 'in_region';
	$need_data['new_region_msg'] = '';
    $need_data['is_drainage'] = 'not_drainage';
    $need_data['is_drainage_msg'] = '';
		...
		$store_trans_free_toal = 0;
		 if($mb_city_id > 0){
			...
			foreach($seller_goodss as $store_id => $val)
			{
                ...
                if(count($val['goods']) == 1 && $val['goods'][0]['is_drainage'] == 1){
                    $need_data['is_drainage'] = 'in_drainage';
                    $need_data['is_drainage_msg'] = '此商品需和商城其他任意商品一起加购物车购买';
                }
                $store_shipping_fare = D('Home/Transport')->setOrderExpress($val['goods'], $mb_city_id );
                if(!$store_shipping_fare['code']){
                    //购物车中没有商品
                    $need_data['new_region'] = 'notin_region';
                    $need_data['new_region_msg'] = $store_shipping_fare['msg'];
                    $trans_free_toal =0.00;
                }else{
                    $trans_free_toal = $store_shipping_fare['data']['express_price'];
                    $store_trans_free_toal = $store_shipping_fare['data']['express_price'];
...
//				}
		...
		}else if(!empty($address) && $mb_city_id == 0)
		{
			...
			foreach($seller_goodss as $store_id => $val)
			{
                ...
                //统一邮费
                $store_shipping_fare = D('Home/Transport')->setOrderExpress($val['goods'], $address['country_id'] );
...
//					}
		...
		//
		if( !empty($man_free_shipping) && $man_free_shipping > 0 && $man_free_shipping <= $store_buy_total_money && $shipping<1)
		{
		...
		if( $member_info['score'] > 0 )
		{
			...
			if( !empty($score_forbuy_money) && $score_forbuy_money >0 )
			{
				...
				if($score_for_money > $max_store_buy_total_money)
				{
					...
					//var_dump($score_for_money,$score_forbuy_money, $bue_use_score );die();
				}
			}
		}
	}
...
public function coin_order(){
    ...
    if( $member_id > 0 )
    {
        $order =  M('lionfish_comshop_coin_order')->where(array('id'=>$order_id,'member_id'=>$member_id))->find();

        if($order){
            if($order['status']){
                echo json_encode( array('code' => 1, 'msg' => '改卡券已经核销') );
                die();
            }
            $data =[
                'status'=>1,
                'hxtime'=>time(),
                'hxname'=>$gpc['ziti_name'],
                'hxmobile'=>$gpc['ziti_mobile'],
                'user_remake'=>$gpc['remake'],
                'community_id'=>$gpc['community_id'],
                'lou_men_hao'=>$gpc['lou_men_hao'],
                'address_name'=>$gpc['address_name'],
                'province_name'=>$gpc['province_name'],
                'city_name'=>$gpc['city_name'],
                'country_name'=>$gpc['country_name'],
                'latitude'=>$gpc['latitude'],
                'longitude'=>$gpc['longitude'],
                'address_type'=>$gpc['address_type'],
                'tuan_send_address'=>$gpc['tuan_send_address'],
                'pickaddress'=>$gpc['pickUpAddress'],
                'disname'=>$gpc['disUserName']
            ];
            $result = M('lionfish_comshop_coin_order')->where(array('id'=>$order_id,'member_id'=>$member_id))->save($data);
            
            $package_order = M('lionfish_comshop_package_order')->where(array('id'=>$order['package_id'],'member_id'=>$member_id))->find();
            /**
             * 更新套餐订单状态，并计算上级或者推荐人佣金
             * 套餐订单：
             *  a.在购买之后，兑换券未兑换时，允许取消订单，因此未兑换时不计算佣金；
             *  b.在兑换任意其中一个券后，订单更新为已核销，不允许取消订单，因此计算佣金；
             *  c.套餐订单的佣金比例，目前采用的是系统统一设定；
             *  d.由于套餐佣金和普通商品佣金比例不一样，目前采用管理员手动分配佣金方式，因此暂时屏蔽套餐佣金计算代码；
             *  e.后续针对套餐增加独立佣金比例，再开启佣金计算代码。
             */
            if(!$package_order['coinstatu']){
                $fenxiao_model = D('Home/Commission');//D('Home/Fenxiao');
                $fenxiao_model->ins_member_package_order($package_order['member_id'],$package_order['id'],0,$package_order['goods_id'],$package_order['agent_id']);
                $package_result = M('lionfish_comshop_package_order')->where(array('id'=>$order['package_id'],'member_id'=>$member_id))->save(['coinstatu'=>1]);
            }
            
            if($result){
                echo json_encode( array('code' => 0, 'msg' => '核销成功') );
                die();
            }
        }else{
            echo json_encode( array('code' => 1, 'msg' => '未找到该卡券信息') );
            die();
        }

    }else{
        echo json_encode( array('code' => 1, 'msg' => '登录失效') );
        die();
    }
}
public function package_order(){
    ...
    if( $member_id > 0 )
    {
        ...
        $member_info = M('lionfish_comshop_member')->where( array('member_id' => $member_id ) )->find();
        if($gpc['agent']>0){
            if($member_id == $gpc['agent']){
                echo json_encode( array('code' => 1, 'msg' => '推荐人不能填写自己') );
                die();
            }
            $agent = M('lionfish_comshop_member')->where( array('member_id' => $gpc['agent'] ) )->find();
            if(!$agent || !$agent['comsiss_state']){
                echo json_encode( array('code' => 1, 'msg' => '用户不存在或不是分销商') );
                die();
            }
        }
        $goods =  M('lionfish_comshop_package_goods')->where(array('id'=>$gpc['goods_id']))->find();
        $order_data = [
            'order_no'=>build_order_no($member_id),
            ...
            'create_time'=>time(),
            'status'=>0,
            'total_money'=>$goods['price'],
          	'openid'=>$member_info['we_openid'],
          	'agent_id'=>$gpc['agent']
        ];
		
        $order_all_id = M('lionfish_comshop_package_order')->add($order_data);
        ...
        $nonce_str =    nonce_str();
        $notify_url =   $shop_domain.'/packnotify.php';

        $openid =       $member_info['we_openid'];
        $out_trade_no = $order_all_id.'-'.time();

        //out_trade_no
        M('lionfish_comshop_package_order')->where( array('id' => $order_all_id ) )->save( array('out_trade_no' => $out_trade_no ) );
      ...
      //$total_fee = 0.01*100;
       ...
       $post['total_fee'] = $total_fee;
        ...
        if($array['RETURN_CODE'] == 'SUCCESS' && $array['RESULT_CODE'] == 'SUCCESS'){
            ...
            M('lionfish_comshop_package_order')->where( array('id' => $order_all_id ) )->save( array('perpay_id' => $prepay_id ) );
			...
			$data['redirect_url'] = '../groups/group?id='.$order_all_id.'&is_show=1';
	...
	**/
	...
	foreach($seller_goodss as $kk => $vv)
	{
		...
		if($dispatching == 'express')
		{
            ...
            if(count($vv) == 1 && $val['goods'][0]['is_drainage'] == 1){
                $need_data['is_drainage'] = 'in_drainage';
                $need_data['is_drainage_msg'] = '此商品需和商城其他任意商品一起加购物车购买';
                echo json_encode( array('code' => 2, 'msg' =>  $need_data['is_drainage_msg'] , 'is_forb' => 1) );
                die();
            }
            $store_shipping_fare = D('Home/Transport')->setOrderExpress($vv, $address_info['country_id']);

            if(!$store_shipping_fare['code']){
                //购物车中没有商品
                $need_data['new_region'] = 'notin_region';
                $need_data['new_region_msg'] = $store_shipping_fare['msg'];
                echo json_encode( array('code' => 2, 'msg' => $store_shipping_fare['msg'] , 'is_forb' => 1) );
                die();
            }else{
                $trans_free_toal = $store_shipping_fare['data']['express_price'];
...
//				}
			...
			if( $kk == 0 && !empty($man_free_shipping) && $man_free_shipping > 0 && $order_goods_total_money >= $man_free_shipping && $shipping<1 )
			{
