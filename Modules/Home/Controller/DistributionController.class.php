...
class DistributionController extends CommonController {
	...
	**/
    ...
    public function get_rule()
    {
        ...
        $solitaire_notice = D('Home/Front')->get_config_by_name('communitymember_traslation_page');
	...
	**/
	public function listorder_list()
	{
        ...
        $sqls = 'select *  from  '
            .C('DB_PREFIX')."lionfish_comshop_member_commiss_order as mco 
			where   mco.member_id=".$member_id." {$where} order by mco.id desc limit {$offset},{$per_page}";


        $list = M()->query($sqls);
        foreach ($list as &$value){
            if($value['order_type']){
                $order_goods = M('lionfish_comshop_package_goods')->where( array('id' => $value['order_goods_id'] ) )->find();
                $order = M('lionfish_comshop_package_order')->where( array('id' => $value['order_id'] ) )->find();
                $value['order_num_alias'] = $order['order_no'];
                $value['total'] = $order['total_money'];
                $value['goods_id'] = $order['goods_id'];
                $value['name'] = $order_goods['goodsname'];
                $value['price'] = $order_goods['price'];
                $value['goods_images'] = $order_goods['big_img'];
            }else{
                $order_goods = M('lionfish_comshop_order_goods')->where( array('order_goods_id' => $value['order_goods_id'] ) )->find();
                $order = M('lionfish_comshop_order')->where( array('order_id' => $value['order_id'] ) )->find();
                $value['order_num_alias'] = $order['order_num_alias'];
                $value['total'] = $order['total'];
                $value['name'] = $order_goods['name'];
                $value['price'] = $order_goods['price'];
                $value['goods_images'] = $order_goods['goods_images'];
                $value['order_status_id'] = $order['order_status_id'];
            }
            $member = M('lionfish_comshop_member')->where( array('member_id' => $value['child_member_id'] ) )->find();
            $value['uname'] = $member['username'];
            $value['quantity'] = 1;
            $value['has_refund_money'] = 0.00;
            $value['has_refund_quantity'] = 0;
...
// 		$list = M()->query($sql);
		...
		//增加小游戏中的订单信息
		if($state == 1 || $state == -1){
		    $gameWhere = "member_id=".$member_id;
		}else{
		    $gameWhere = "member_id=".$member_id." and state = 999";
		}
		$game_good_order_list = M('lionfish_comshop_minigame_commiss_order')->where($gameWhere)->order('id desc')
		->limit($offset, $per_page)->select();
		if (count($game_good_order_list) > 0) {
		    foreach ($game_good_order_list as  $game_data){
		        
		        $game_val['total'] = round($game_data['goodsprice'],2);
		        
		        $game_val['money'] = round($game_data['money'],2);
		        
		        $game_val['price'] = sprintf("%.2f", $game_data['goodsprice']);
		        
		        $game_val['status_name'] = '已支付';     
		        
		        $game_val['addtime'] = date('Y-m-d', $game_data['addtime']);
		        
		        $str='1234567890';
		        $randStr = str_shuffle($str);//打乱字符串
		        $rands= substr($randStr,0,9);
		        
		        $game_val['order_num_alias'] = date('Ymd', $game_data['addtime']).$rands;
		        
		        $game_val['goods_images']= 'https://yyg.yijiaegou.com/Uploads/image/goods/2020-10-17/5f8a48c38600e.jpg'; 
		        
		        //$game_val['goods_images']= $game_data['goodsimage'];
		        
		        $game_val['option_str'] = '';
		        
		        $game_val['old_commision'] = $game_data['money'];
		        
		        $game_val['del_commision'] = 0;
		        
		        $game_val['member_id'] = $member_id;
		        
		        $game_val['level'] = 1;
		        
		        $game_val['quantity'] = $game_data['goodsnum'];
		        
		        $game_val['old_commision'] = $game_data['money'];
		        
		        $game_val['name'] = $game_data['goodsname'];
		        
		        $game_val['uname'] = $game_data['username'];
		        
		        $game_val['state'] = 1;
		        
		        array_push($list, $game_val);
                $addTimes = array_column($list, 'addtime');
                array_multisort($addTimes, SORT_DESC, $list);
		    }
	...
	//后台添加团队长佣金收益流水profit_flow()
	
	public function get_profitflow()
	{
	    $_GPC = I('request.');
	    $token = $_GPC['token'];
        ...
        */
	    $list = M('lionfish_comshop_member_profit_flow')->where( array('member_id' => $member_id) )->order('id desc')->select();
		foreach( $list as $key => $val )
		{
			$val['addtime'] = date('Y-m-d H:i',$val['addtime'] );	
