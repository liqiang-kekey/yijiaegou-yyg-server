...
class IndexController extends CommonController {
	...
	public function vip_info(){
        ...
        $nav = M('lionfish_comshop_generalmall_adv')->where( array('enabled' => 1,'type'=>'recipe') )->order('displayorder desc, id desc')->select();
        if(!empty($nav))
        {
        ...
        $business = M('lionfish_comshop_generalmall_adv')->where( array('enabled' => 1,'type'=>'pintuan') )->order('displayorder desc, id desc')->select();
        if(!empty($business))
        {
        ...
        //oscshop_lionfish_comshop_good_common
        $vipgoods  =  M('lionfish_comshop_good_common')->where( array('is_limit_levelunbuy' => 1) )->select();
        $list  = [];
        if($vipgoods){
            foreach ($vipgoods as $val) {
                $temp_array[] = $val['goods_id'];
            }
            // 满减
	...
	public function index_info()
	{
		...
		**/
		...
		//首页顶部文字背景图片
		$index_header_text_backgroundimage = D('Home/Front')->get_config_by_name('index_header_text_backgroundimage');
		if( !empty($index_header_text_backgroundimage) )
			{
				$index_header_text_backgroundimage = tomedia($index_header_text_backgroundimage);
			}
		//直播相关信息index_header_zhibo_image
		$index_header_zhibo_image = D('Home/Front')->get_config_by_name('index_header_zhibo_image');
		if( !empty($index_header_zhibo_image) )
		{
			$index_header_zhibo_image = tomedia($index_header_zhibo_image);
		}
		
		$is_show_goods_classify_number = D('Home/Front')->get_config_by_name('is_show_goods_classify_number');
		...
		echo json_encode(array('code'=>0,
						...
						'index_header_text_backgroundimage' => $index_header_text_backgroundimage,
						'ishide_index_goodslist' => $ishide_index_goodslist,
						'index_header_zhibo_image' => $index_header_zhibo_image,
		                'is_show_goods_classify_number' => $is_show_goods_classify_number
					)
			);
		die();
	}
	...
	**/
		...
		if( !empty($head_id) && $head_id >0 )
		{
			if($gid == 0){
				...
				$sql_goods_ids = "select pg.goods_id from ".C('DB_PREFIX')."lionfish_community_head_goods as pg,"
        	           ...
        	           where  pg.goods_id = g.goods_id and g.cate_id in ({$gidstr}) and pg.head_id = {$head_id}  order by pg.id desc ";
			...
			if($gid == 0){
				...
				$goods_ids_nolimit_sql = "select pg.id from ".C('DB_PREFIX')."lionfish_comshop_goods as pg,"
                        .C('DB_PREFIX')."lionfish_comshop_goods_to_category as g  
        	           where  pg.id = g.goods_id and g.cate_id in ({$gidstr}) and pg.is_all_sale=1 ";
	...
	public function load_package_coindetail(){
        $gpc = I('request.');
        $token = $gpc['token'];
        $id = $gpc['id'];
        $coin =  M('lionfish_comshop_coin_order')->where(array('id'=>$id))->find();
        $list = M('lionfish_comshop_goods')->where(array('id'=>$coin['goods_id']))->find();
        ...
        $need_data['ziti_name'] = $address['name'];
        $need_data['ziti_mobile'] = $address['telephone'];
        $need_data['address'] = $address;
        if($coin){
            if( !empty($coin['goods_img']) )
            {
                $coin['goods_imgs'] = tomedia($coin['goods_img']);
            }
            $coin['price'] = $list['price'];
            echo json_encode( array('code' => 0 ,'coin'=>$coin,'address'=>$need_data) );
        }
        die();
    }
	public function load_package_goodsdetail(){
        $gpc = I('request.');
        $token = $gpc['token'];
        $id = $gpc['id'];
        $list = M('lionfish_comshop_package_goods')->where(array('id'=>$id))->find();
        if($list){
            if( !empty($list['big_img']) )
            {
                $list['bigImg'] = tomedia($list['big_img']);
            }
            if( !empty($list['thumbs']) )
            {
                $list['thumb'] = tomedia($list['thumbs']);
            }
            ...
            //$coin = M('lionfish_comshop_coin')->where(array('card_id'=>$id))->select();
          	$coin = M('lionfish_comshop_coin')->where(array('card_id'=>$id,'status'=>1))->select();
            foreach ($coin as &$val){
                if( !empty($val['goods_img']) )
                {
                    $val['goods_imgs'] = tomedia($val['goods_img']);
                }

            }
            echo json_encode( array('code' => 0 , 'list' => $list,'coin'=>$coin) );
        }

        die();
    }
    //获取指定套餐兑换卡
    public function load_package_mycoinlist(){
        $gpc = I('request.');
        $id = $gpc['id'];
        $token = $gpc['token'];
        $weprogram_token = M('lionfish_comshop_weprogram_token')->field('member_id')->where( array('token' => $token) )->find();
        ...
        $list = M('lionfish_comshop_coin_order')->where(array('package_id'=>$id,'member_id'=>$member_id))->select();
        if($list){
            foreach ($list as &$val){
                if( !empty($val['addtime']) )
                {
                    $val['addtimes'] = date('Y-m-d h:i:s',$val['addtime']);
                }
            }
    ...
    //获取套餐兑换卡
    public function load_package_coinlist(){
        $gpc = I('request.');
        ...
        $member_id = $weprogram_token['member_id'];
        //$member_info = M('lionfish_comshop_member')->where( array('member_id' => $member_id ) )->find();
        $list = M('lionfish_comshop_coin_order')->where(array('status'=>0,'member_id'=>$member_id))->select();
        if($list){
            foreach ($list as &$val){
                if( !empty($val['addtime']) )
                {
    ...
    public function load_package_refound_order(){
        $gpc = I('request.');
        ...
        $member_id = $weprogram_token['member_id'];
        //$member_info = M('lionfish_comshop_member')->where( array('member_id' => $member_id ) )->find();
        $list = M('lionfish_comshop_package_order')->where(array('member_id'=>$member_id,'id'=>$id))->find();

        $coinorder = M('lionfish_comshop_coin_order')->where(array('member_id'=>$member_id,'package_id'=>$id))->select();
        if(!$coinorder){
            echo json_encode( array('code' => 1 , 'msg' => '为找到兑换券') );
        }
        foreach ($coinorder as $key=>$val){
            if($val['status'] == 1){
                echo json_encode( array('code' => 1 , 'msg' => '已有兑换券核销,无法申请退款') );
            }
        }
        if($list['status'] != 1){
            echo json_encode( array('code' => 1 , 'msg' => '状态发生改变') );
        }
        if($list){
            $data = [
                'status'=>2,
            ...
            $result = M('lionfish_comshop_package_order')->where(array('member_id'=>$member_id,'id'=>$id))->save($data);
          $res = M('lionfish_comshop_coin_order')->where(array('member_id'=>$member_id,'package_id'=>$id))->save(['status'=>3]);
           if($result && $res){
                echo json_encode( array('code' => 0 , 'msg' => '已提交退款审核中') );
            }else{
                echo json_encode( array('code' => 1 , 'msg' => '网络异常') );
            }
        }

        die();
    }
    public function load_package_order(){
        $gpc = I('request.');
        ...
        $member_id = $weprogram_token['member_id'];
        //$member_info = M('lionfish_comshop_member')->where( array('member_id' => $member_id ) )->find();
        //$list = M('lionfish_comshop_package_order')->where(array('member_id'=>$member_id))->select();
		$map['status'] = array('EGT',1);
        $map['member_id'] = $member_id;
        $list = M('lionfish_comshop_package_order')->where($map)->order('id desc')->select();
        if($list){
            $coinstatus = 1;
            foreach ($list as &$val){
                $coinorder = M('lionfish_comshop_coin_order')->where(array('member_id'=>$member_id,'package_id'=>$val['id']))->select();
                
                foreach ($coinorder as $k=>$v){
                    if(!$v['status']){
                        $coinstatus = 0;
                    }else{
                        $coinstatus = 1;
    ...
    public function load_package_goodslist(){
        $gpc = I('request.');

        $token = $gpc['token'];
        $list = M('lionfish_comshop_package_goods')->where(array('status'=>1))->select();
        if($list){
            foreach ($list as &$val){
                if( !empty($val['big_img']) )
                {
                    $val['bigImg'] = tomedia($val['big_img']);
                }
                if( !empty($val['thumbs']) )
                {
                    $val['thumb'] = tomedia($val['thumbs']);
                }
            }
            echo json_encode( array('code' => 0 , 'list' => $list) );
        }
	...
	**/
	...
	public function load_cate_goodslist()
	{
		...
		$parent_cate = M('lionfish_comshop_goods_category')->field('id,banner,name,is_show_prolayout')->where( array('cate_type' =>'normal','id' => $pid ) )->find();
		$is_show_prolayout = 0;
		
	    if($parent_cate){
	    	$is_show_prolayout = $parent_cate['is_show_prolayout'];
	...
	//获取小程序直播相关信息
	public function get_zhibo_info()
	{
		//
    	$data = D('Seller/Config')->get_all_config();
		$appid = $data['wepro_appid'];
        $appsecret = $data['wepro_appsecret'];
		...
		$para = json_encode(array('start'=>0,'limit'=>50));
		$resdata = $this->http_request($url,$para);
		$jsondata=json_decode($resdata,true);
		$data = $jsondata["room_info"];
		$errcode = $jsondata["errcode"];
		$showzbimg = false;
		$roomid = 0;
		if($errcode == 0)
		{
			foreach($data as $v_zb)
			{
				if(($v_zb["live_status"] == 101)or($v_zb["live_status"] == 102))
				{
					$showzbimg = true;
					$roomid = $v_zb["roomid"];
					break;
		...
		echo json_encode( array('showzbimg' => $showzbimg , 'roomid' => $roomid ) );
		//print_r($resdata);
		//echo json_encode($data);
		die();
	}
	/*
	{"errcode":0,"errmsg":"ok","room_info":[{"name":"直播测试4","cover_img":"http:\/\/mmbiz.qpic.cn\/mmbiz_jpg\/7Tl1QOCCpLWQEY8E9vkcJ8K7EmH5Ao4wxVia4G0H1UzFWpvWOIQZ6HYZ55mNyy6KRymT7XZOXZiaZGJRhhcgibT2A\/0","start_time":1593595422,"end_time":1593596723,"anchor_name":"小吴","roomid":4,"goods":[],"live_status":103,"share_img":"http:\/\/mmbiz.qpic.cn\/mmbiz_jpg\/7Tl1QOCCpLWQEY8E9vkcJ8K7EmH5Ao4wxVia4G0H1UzFWpvWOIQZ6HYZ55mNyy6KRymT7XZOXZiaZGJRhhcgibT2A\/0"},{"name":"直播测试三","cover_img":"http:\/\/mmbiz.qpic.cn\/mmbiz_jpg\/7Tl1QOCCpLVr0avjeCQHgtY9Fp1UVibibSRBWsYfu5p6OTOIpice1rgAa7ROn6rtricHvKlX0wNtqfS2ibsDxSNYibww\/0","start_time":1593512305,"end_time":1593512672,"anchor_name":"小吴","roomid":3,"goods":[],"live_status":103,"share_img":"http:\/\/mmbiz.qpic.cn\/mmbiz_jpg\/7Tl1QOCCpLVr0avjeCQHgtY9Fp1UVibibSdPOgH37DRygLwUXUnia1nAOV9IlDs4Lw8GmLCcEicZN9CIrvvtXTibHSA\/0"},{"name":"直播测试二","cover_img":"http:\/\/mmbiz.qpic.cn\/mmbiz_jpg\/7Tl1QOCCpLVr0avjeCQHgtY9Fp1UVibibS9T9tYgsGOQrwQPJPlQUnaZBOKTtQzU6jAV5BBBAnIMMnl1u3RFgejg\/0","start_time":1593511790,"end_time":1593512018,"anchor_name":"主播昵称","roomid":2,"goods":[],"live_status":103,"share_img":"http:\/\/mmbiz.qpic.cn\/mmbiz_jpg\/7Tl1QOCCpLVr0avjeCQHgtY9Fp1UVibibSMsWKo9kuYNiahFEsyHeZohjLnVv23TMrOr4tnibgfeH6XIyh5jzPLcrw\/0"}],"total":3,"live_replay":[]}
	*/
	//
	
	public function http_post($url, $param=array())
	{
    if(!is_array($param)){
...
//        print_r($access_token);die;
    ...
    //溯源直播请求测试
    public function get_suyuan_info()
    {
    	$url = "https://cdn88.cn/api/";
		$para = array('cmdId'=>100,'user'=>'sxyk','password'=>'');
		$resdata = $this->http_request($url,$para);
		$jsondata=json_decode($resdata,true);
		$cmdid = $jsondata["cmdId"];
		$devlist = $jsondata["devlist"];
		//五常大米,括苍山日出广角镜头直播,办公室广场直播
		/*
		$name="办公室广场直播";
		$resdata2 = array();
		foreach($devlist as $val)
		{
			if($val['name'] == $name)
			{
				$resdata2 = array('id'=>$val['id'],
								 'sn'=>$val['sn'],
								 'name' =>$val['name'],
								 'hlsurl'=>$val['hlsurl'],
								 'imgsrc'=>$val['imgsrc'],
		...
		*/
		//
		//echo json_encode( array('cmdid'=>$cmdid,'devlist' => $devlist ) );
		echo json_encode($devlist);
		die();
    }
    //获取指定设置的摄像头信息
    public function get_camera_info()
    {
    	$_GPC = I('request.');
		$devsn = $_GPC['devsn'];
		//
    	$url = "https://cdn88.cn/api/";
    	$para = array('cmdId'=>210,'user'=>'demo','dev'=>$devsn);
    	$resdata = $this->http_request($url,$para);
		$jsondata=json_decode($resdata,true);
    ...
    */
    //获取溯源摄像头图片介绍
    public function get_suyuan_pic()
    {
        //溯源相关信息suyuan_image
		$suyuan_image1 = D('Home/Front')->get_config_by_name('suyuan_image1');
		if( !empty($suyuan_image1) )
		{
			$suyuan_image1 = tomedia($suyuan_image1);
		}
		//
		$suyuan_image2 = D('Home/Front')->get_config_by_name('suyuan_image2');
		if( !empty($suyuan_image2) )
		{
			$suyuan_image2 = tomedia($suyuan_image2);
		}
		//
		$suyuan_image3 = D('Home/Front')->get_config_by_name('suyuan_image3');
		if( !empty($suyuan_image3) )
		{
			$suyuan_image3 = tomedia($suyuan_image3);
		}
		//
		echo json_encode(array('code' => 0, 'image1' => $suyuan_image1,'image2' => $suyuan_image2,'image3' => $suyuan_image3));
			die();
    }
}