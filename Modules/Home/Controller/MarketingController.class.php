...
class MarketingController extends CommonController {
    ...
    public function get_business_list(){
        ...
        if($category){
            foreach ($category as &$item){
                $item['list'] = M('lionfish_comshop_business')->where( array('category_id' => $item['id']) )->select();
            ...
            echo json_encode( array('code' => 0, 'data' => $category ) );
            die();
        }else{
            echo json_encode( array('code' => 1, 'msg' => '无商圈') );
            die();
        }

    }
    //海报 信息
    public function get_poster_index(){
        $_GPC = I('request.');
        $token =  $_GPC['token'];
        $category_id = $_GPC['category_id'];
        $list = M('lionfish_comshop_poster')->where( array('enabled' => 1,'category_id'=>$category_id) )->select();
        ...
        if($list){
            foreach ($list as &$val){
                ...
                if( !empty($val['thumb']) )
                {
                    $val['thumbs'] = tomedia($val['thumb']);
                }
                if( !empty($val['video']) )
                {
                    $val['videos'] = tomedia($val['video']);
                }
            }
            echo json_encode( array('code' => 0 , 'list' => $list) );
            die();
        }else{
            echo json_encode( array('code' => 1, 'msg' => '无详情') );
            die();
        }
    ...
    public function get_business_detail(){
        ...
        $token =  $_GPC['token'];
        $id = $_GPC['id'];
        $list = M('lionfish_comshop_business')->where( array('id' => $id) )->find();
        if($list){
