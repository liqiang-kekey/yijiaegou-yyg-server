	...
	**/
	...
	public function updatePintuanSuccess($pin_id)
	{
	    ...
	    foreach($pin_order_arr as $pin_order)
	    {
			...
			$pagepath = 'community/moduleA/pin/share?id='.$pin_order['order_id'];
			...
			if(!empty($template_order_success_notice))
			{
				...
				{
					$head_pathinfo = "community/moduleA/groupCenter/groupDetail?groupOrderId=".$pin_order['order_id'];
	...
	**/
	...
	function insertNotifyOrder($order_id)
	{
		...
		if( $is_tuanz )
		{
			...
			$pagepath = 'community/moduleA/pin/share?id='.$order_id;
			...
			$member_info = M('lionfish_comshop_member')->where( array('member_id' => $order_info['member_id'] ) )->find();
				...
				if( !empty($mb_subscribe) )
				{
					...
					$template_data['amount3'] = array('value' => sprintf("%01.2f", $price) );
