...
class MarketingController extends CommonController{
	...
	public function lottery_list(){
        $_GPC = I('request.');
      	 ...
      	 $catid = $_GPC['catid'];
        $gpc = $catid;
        $category = M('lionfish_comshop_lottery_category')->select();
        $this->category = $category;
        $this->gpc = $gpc;
        ...
        $sql = 'SELECT * FROM ' . C('DB_PREFIX'). 'lionfish_comshop_lottery_list ' . ' where 1 and ' . $condition . ' ORDER BY id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
        $list = M()->query($sql);
        foreach ($list as &$item){
            $categoryname =  M('lionfish_comshop_lottery_category')->where( array('id' => $item['category_id']) )->find();
            if($categoryname){
                $item['categoryname'] =$categoryname['name'];
            }else{
                $item['categoryname'] = '活动记录已删除';
            }

        }

        $total_arr = M()->query('SELECT COUNT(*) as count FROM ' . C('DB_PREFIX'). 'lionfish_comshop_lottery_list where 1 and ' . $condition );
    ...
    public function lottery_config()
    {
        $_GPC = I('request.');
	...
	public function recharge_diary()
	{
        ...
        if($export == 1){
            $sql = "select cf.* ,m.username,m.avatar  from ".C('DB_PREFIX')."lionfish_comshop_member_charge_flow as cf , ".C('DB_PREFIX')."lionfish_comshop_member as m 
				where cf.member_id = m.member_id {$condition} order by cf.id desc ";
            $list = M()->query($sql);
            if ($list) {
                foreach( $list as $key => $val )
                {
                    ...
                    if($val['state'] == 1)
                    {
                        ...
                        $val['pay_type'] = '分销提现到余额';
                        $val['money'] = '+'.$val['money'];
                    }elseif ($val['state'] == 10){
                        $val['pay_type'] = '团长提现到余额';
                        $val['money'] = '+'.$val['money'];
                    }
                    if($val['state'] == 3 || $val['state'] == 4)
                    {
            ...
            @set_time_limit(0);
            $columns = array(
                array('title' => '用户名', 'field' => 'username', 'width' => 24),
                array('title' => '金额', 'field' => 'money', 'width' => 24),
                array('title' => '账户余额', 'field' => 'operate_end_yuer', 'width' => 12),
                array('title' => '操作时间', 'field' => 'add_time', 'width' => 12),
                array('title' => '交易类型', 'field' => 'pay_type', 'width' => 12),
            );
            $fileName = date('YmdHis', time());
            D('Seller/Excel')->export($list, array('title' => '充值流水数据'.$fileName, 'columns' => $columns));
        }
	...
	**/
	...
	public function editlotterylist(){
        $_GPC = I('request.');
        ...
        if (IS_POST) {
            ...
            $data = array();
            $data['remake'] = $_GPC['remake'];
            $data['status'] = 1;
            $data['addtime'] = time();
            if($id > 0)
            {
                M('lionfish_comshop_lottery_list')->where( array('id' => $id) )->save( $data );
            }else{
                $id = M('lionfish_comshop_lottery_list')->add($data);
            }

            show_json(1, array('url' => $_SERVER['HTTP_REFERER']));
        }
        $item = M('lionfish_comshop_lottery_list')->where( array('id' => $id ) )->find();
    ...
    public function addlottery ()
    {
        ...
        $id = intval($_GPC['id']);
        $lotterylist =  M('lionfish_comshop_lottery_category')->where( array('status' => 1) )->select();
        ...
        if (IS_POST) {
            ...
            $data['name'] = $_GPC['name'];
            $data['img'] = $_GPC['img'];
            $data['luck'] = intval($_GPC['luck']);
            $data['category_id'] = intval($_GPC['category_id']);
            if($id > 0)
            {
                M('lionfish_comshop_lottery')->where( array('id' => $id) )->save( $data );
            }else{
                $data['addtime'] = time();
                $id = M('lionfish_comshop_lottery')->add($data);
            }

            show_json(1, array('url' => $_SERVER['HTTP_REFERER']));
        }
        $item = M('lionfish_comshop_lottery')->where( array('id' => $id ) )->find();
    ...
    //删除活动记录
    public function deletelottery_category(){
        $_GPC = I('request.');
        $id = intval($_GPC['id']);
        ...
        $items = M()->query('SELECT id FROM ' . C('DB_PREFIX'). 'lionfish_comshop_lottery_category WHERE id in( ' . $id . ' ) ');

        foreach ($items as $item ) {
            M('lionfish_comshop_lottery_category')->where( array('id' => $item['id']) )->delete();
        }

        show_json(1, array('url' => $_SERVER['HTTP_REFERER']));
    }
    
    public function  changelottery_category(){
        $_GPC = I('request.');
        $id = intval($_GPC['id']);
        $data['status'] = intval($_GPC['status']);
        
        if($data['status'] == 1){
            $categoryList = M('lionfish_comshop_lottery_category')->where(array('status' => 1))->select();
            if(!empty($categoryList)){
                foreach ($categoryList as $itemCate){
                    $itemCate['status'] = 0;
                    M('lionfish_comshop_lottery_category')->save($itemCate);
                }
            }
        }
        
        $categoryList = M('lionfish_comshop_lottery_category')->where(array('id' => $id))->select();
        if(!empty($categoryList)){
            foreach ($categoryList as $itemCate){
                $itemCate['status'] = $data['status'];
                M('lionfish_comshop_lottery_category')->save($itemCate);
            }
        }
        show_json(1, array('url' => $_SERVER['HTTP_REFERER']));
    ...
    //添加活动记录
    public function addlottery_category(){
        $_GPC = I('request.');
        ...
        if (IS_POST) {
            ...
            if($id > 0)
            {
                M('lionfish_comshop_lottery_category')->where( array('id' => $id) )->save( $data );
            }else{
                $data['addtime'] = time();
                $id = M('lionfish_comshop_lottery_category')->add($data);
            }

            show_json(1, array('url' => $_SERVER['HTTP_REFERER']));
        }
        $item = M('lionfish_comshop_lottery_category')->where( array('id' => $id ) )->find();

        if (empty($item)) {
            $start_time =  time();
            $end_time = strtotime(date('Y-m-d H:i:s', $starttime) . '+7 days');
            $starttime = time();
            $endtime = strtotime(date('Y-m-d H:i:s', $starttime) . '+7 days');
        }else {
    ...
    //活动记录
    public function lottery_category(){
        $_GPC = I('request.');
        ...
        $sql = 'SELECT * FROM ' . C('DB_PREFIX'). 'lionfish_comshop_lottery_category ' . ' where 1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
        $list = M()->query($sql);


        $total_arr = M()->query('SELECT COUNT(*) as count FROM ' . C('DB_PREFIX'). 'lionfish_comshop_lottery_category where 1 and ' . $condition );
	...
	public function lottery(){
        ...
        $sql = 'SELECT * FROM ' . C('DB_PREFIX'). 'lionfish_comshop_lottery ' . ' where 1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
        $list = M()->query($sql);


        $total_arr = M()->query('SELECT COUNT(*) as count FROM ' . C('DB_PREFIX'). 'lionfish_comshop_lottery where 1 and ' . $condition );

        $total = $total_arr[0]['count'];

        $pager = pagination2($total, $pindex, $psize);
        foreach ($list as &$item){
            $lotterylist =  M('lionfish_comshop_lottery_category')->where( array('id' => $item['category_id']) )->find();
            $item['category'] = $lotterylist['name'];
        }
        $this->list = $list;
        $this->pager = $pager;
    ...
    public function deletelotterylist()
    {
        $_GPC = I('request.');
        $id = intval($_GPC['id']);
        ...
        $items = M()->query('SELECT id FROM ' . C('DB_PREFIX'). 'lionfish_comshop_lottery_list WHERE id in( ' . $id . ' ) ');

        foreach ($items as $item ) {
            M('lionfish_comshop_lottery_list')->where( array('id' => $item['id']) )->delete();
        }
