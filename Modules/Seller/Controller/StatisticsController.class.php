    ...
    public function index_data()
	{
		...
		switch( $type )
		{
			case 'normal':
                ...
                $today_package_where = " status = 1 and create_time > {$today_begin_time}";
                $package_count = M('lionfish_comshop_package_order')->where($today_package_where)->sum('pay_money');
                $package_order_count = M('lionfish_comshop_package_order')->where($today_package_where)->count('id');
				if (defined('ROLE') && ROLE == 'agenter' ) 
				{
					...
					$result['today_pay_order_count'] = count($order_ids_arr)+$package_order_count;
					...
					$today_pay_money = $today_pay_money+$package_count;
					...
					$result['today_pay_order_count'] = $today_pay_order_count+$package_order_count;
				...
				if (defined('ROLE') && ROLE == 'agenter' ) 
				{
					...
					**/
                    ...
                    $package_count_count = M('lionfish_comshop_package_order')->where(array('status'=>1))->sum('pay_money');
					$total_order_money = $total_order_money_arr[0]['total']+$package_count_count;
