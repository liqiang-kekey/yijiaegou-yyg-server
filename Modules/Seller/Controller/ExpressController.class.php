...
class ExpressController extends CommonController{
    ...
    public function fullfree()
    {
        $_GPC = I('request.');

        if (IS_POST) {
            $parameter = ((is_array($_GPC['parameter']) ? $_GPC['parameter'] : array()));

            $data['man_free_shipping'] = trim($parameter['man_free_shipping']);
            $data['delivery_type_express'] = $parameter['delivery_type_express'];
            $data['delivery_express_name'] = $parameter['delivery_express_name'];
            $data['placeorder_trans_name'] = $parameter['placeorder_trans_name'];
            $data['full_free_limit_goods_list'] = $_GPC['limit_goods_list'];
            $data['freight_rule'] = $parameter['freight_rule'];
            ...
            if (is_array($randoms)) {
                foreach ($randoms as $random) {
                    ...
                    $areas = array('citys' => $_GPC['citys'][$random], 'citys_code' => $_GPC['citys_code'][$random]);
                }
            }

            if(!empty($areas)){
                $data['full_free_areas'] = serialize($areas);
            }else{
                $data['full_free_areas'] = '';
            }
        ...
        $limit_goods = array();
        if(!empty($data['full_free_limit_goods_list'])){

            $goodsids_arr = explode(',', $data['full_free_limit_goods_list'] );
        ...
        $this->id =$id;
        $dispatch_areas[] = unserialize($data['full_free_areas']);
        $this->limit_goods = $limit_goods;
