...
class ConfigindexController extends CommonController{
    ...
    public function posterlist(){
        ...
        $list = M()->query('SELECT * FROM ' . C('DB_PREFIX'). "lionfish_comshop_posterlist 
				WHERE 1   " . $condition . ' order by displayorder desc, id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize);

        $total = M('lionfish_comshop_posterlist')->where('1 '. $condition )->count();
    ...
    public function addposterlist()
    {
        ...
        if (!empty($id)) {
            ...
            $item = M('lionfish_comshop_posterlist')->where( array('id' => $id) )->find();
            $this->item = $item;
        }
        ...
        if (IS_POST) {
            ...
            if( !empty($id) && $id > 0 )
            {
                ...
                M('lionfish_comshop_posterlist')->where( array('id' => $id) )->save( $ins_data );

                $id = $data['id'];
            }else{
                $id = M('lionfish_comshop_posterlist')->add( $ins_data );
            }
    ...
    public function changeposterlist()
    {
        $_GPC = I('request.');
        $id = intval($_GPC['id']);
        ...
        $items = M('lionfish_comshop_posterlist')->field('id')->where('id in( ' . $id . ' ) ')->select();

        foreach ($items as $item) {
            M('lionfish_comshop_posterlist')->where( array('id' => $item['id']) )->save( array($type => $value) );
        }
    ...
    public function deleteposterlist()
    {
        ...
        foreach ($items as $item) {
            M('lionfish_comshop_posterlist')->where( array('id' => $item['id']) )->delete();
        }
    ...
    public function poster()
    {
        ...
        $list = M()->query('SELECT * FROM ' . C('DB_PREFIX'). "lionfish_comshop_poster 
				WHERE 1   " . $condition . ' order by displayorder desc, id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize);

        $total = M('lionfish_comshop_poster')->where('1 '. $condition )->count();
    ...
    public function addposter()
    {
        ...
        $posterlist =  M('lionfish_comshop_posterlist')->where( array('enabled' => 1) )->select();
        $category = D('Seller/GoodsCategory')->getFullCategory(false, true);
        $this->category = $category;
        ...
        if (!empty($id)) {
            // $category = D('Seller/GoodsCategory')->getFullCategory(false, true);
            // $this->category = $category;
            $item = M('lionfish_comshop_poster')->where( array('id' => $id) )->find();
            $item['coupon_id'] = explode(',',$item['coupon_id']);
            $this->item = $item;
        }
        $coupon = M('lionfish_comshop_coupon')->field('voucher_title,id')->select();
        $this->coupon = $coupon;
        
        if (IS_POST) {
            $data = $_GPC['data'];
            if(isset($_GPC['cate_mult'])){
                if(!empty($_GPC['cate_mult'])){
                    $data['coupon_id'] = implode(',',$_GPC['cate_mult']);
                }
            ...
            $ins_data['types'] = $data['types'];
            $ins_data['video'] = $data['video'];
            $ins_data['coupon_id'] = $data['coupon_id'];
            $id = $data['id'];
            if( !empty($id) && $id > 0 )
            {
                unset($ins_data['addtime']);
                M('lionfish_comshop_poster')->where( array('id' => $id) )->save( $ins_data );

                $id = $data['id'];
            }else{
                $id = M('lionfish_comshop_poster')->add( $ins_data );
            }
            show_json(1, array('url' => $_SERVER['HTTP_REFERER']));
        }
        $this->display();
    ...
    public function changeposter()
    {
        ...
        $items = M('lionfish_comshop_poster')->field('id')->where('id in( ' . $id . ' ) ')->select();

        foreach ($items as $item) {
            M('lionfish_comshop_poster')->where( array('id' => $item['id']) )->save( array($type => $value) );
        }
    ...
    public function deleteposter()
    {
        ...
        foreach ($items as $item) {
            M('lionfish_comshop_poster')->where( array('id' => $item['id']) )->delete();
        }
    ...
    public function advertisementlist()   
    {
        ...
        if (!empty($keyword)) {
            $condition .= ' and advmname like ' . '"%' . $keyword . '%"';
        }
        ...
        $list = M()->query('SELECT id,advmname,thumbimg,link,displayorder,enabled FROM ' .
            C('DB_PREFIX') . "lionfish_comshop_advertisement  \r\n
		WHERE  " . $condition . ' order by displayorder desc, id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize);

        $total = M('lionfish_comshop_advertisement')->where($condition)->count();
    ...
    public function addadvertisement()
    {
        ...
        $id = I('request.id');
        if (!empty($id)) {
            $item       = M('lionfish_comshop_advertisement')->where(array('id' => $id))->find();
            $this->item = $item;
        }
        ...
        if (IS_POST) {
            $data = I('request.data');
            /**
             * 因为首页广告同时只能开启一个，因此关闭其他的广告
             */
            if($data['enabled'] == 1){
                $itemsDatas = M('lionfish_comshop_advertisement')->where("enabled=1")->order('displayorder desc')->select();
                foreach ($itemsDatas as $itemData) {
                    M('lionfish_comshop_advertisement')->where(array('id' => $itemData['id']))->save(array('enabled' => 0));
                }
            }
            
            D('Seller/Advertisement')->saveOrUpdate($data);
            show_json(1, array('url' => $_SERVER['HTTP_REFERER']));
        }
    ...
    public function changeadvertisement()
    {
        ...
        $items = M('lionfish_comshop_advertisement')->where(array('id' => array('in', $id)))->select();
        foreach ($items as $item) {
            M('lionfish_comshop_advertisement')->where(array('id' => $item['id']))->save(array($type => $value));
        }
    ...
    public function deleteadvertisement()
    {
        ...
        foreach ($items as $item) {
            M('lionfish_comshop_advertisement')->where(array('id' => $item['id']))->delete();
        }
