...
class DistributionController extends CommonController {
	...
	public function communityorder()
	{
		...
		$sql = "select co.order_id,co.state,co.money,co.level,co.order_type,co.addtime ,og.total,og.name,og.total     
		...
		if( !empty($list) )
		{
			...
			{
			    if($val['order_type']){
			        $name = M('lionfish_comshop_package_order')->where( array('id' => $val['order_id'] ) )->find();
			        $val['name'] =$name['goods_name'];
                    $val['total'] = sprintf("%.2f",$name['total_money']);
                }else{
