...
class PackagesController extends CommonController{

    protected function _initialize(){
        parent::_initialize();
        $this->breadcrumb1='套餐管理';
        $this->breadcrumb2='套餐商品信息';
    ...
    public function index(){
        ...
        if (!(empty($keyword))) {
            $condition .= " WHERE  (`id` = '{$keyword}' or `goodsname` LIKE '%{$keyword}%' or `codes` LIKE '%{$keyword}%' ) ";
        }
        if( !empty($starttime_arr) )
        {
            $condition .= ' WHERE  (addtime >='.$starttime.' and addtime <= '.$endtime.' )';
        }

        $sql = 'SELECT COUNT(id) as count FROM ' .C('DB_PREFIX'). 'lionfish_comshop_package_goods ' .$sqlcondition.  $condition ;
        ...
        $sort_way = '`id` DESC';
        if (!(empty($total))) {
            $sql = 'SELECT * FROM ' .C('DB_PREFIX'). 'lionfish_comshop_package_goods '  .$sqlcondition . $condition . ' 
					ORDER BY   '.$sort_way.' ';
...
//        dump($list);die;
    ...
    public function settime()
    {
  	 ...
  	 public function refound(){
        $id = I('get.id');
        $list = M('lionfish_comshop_package_order')->where(array('id'=>$id))->find();
        $http_refer = $_SERVER['HTTP_REFERER'];
        if($list['status'] != 2){
            show_json(0, array('message' => '订单异常无法退款！','url' => $http_refer ));
        }
        $weixin_model = D('Home/Weixin');
        $res = $weixin_model->refundPackageOrder($id);
        if( $res['code'] == 0 )
        {
            ...
            //状态0=待核销.1=已核销,2=订单完成,3=冻结中,4=已失效
            $result = M('lionfish_comshop_package_order')->where(array('id'=>$id))->save($data);
            $res = M('lionfish_comshop_coin_order')->where(array('package_id'=>$id))->save(['status'=>4]);
              
            if($res && $result){
                show_json(1, array('message' => '退款成功！','url' => $http_refer) );
            }else{
             	show_json(0, array('message' => '订单异常无法退款！','url' => $http_refer ));
            }
    ...
    public function order(){
        ...
        if (!(empty($keyword))) {
            if(!empty($searchfield)){
                ...
                $condition .= " AND  (`id` = '{$keyword}' or `goods_name` LIKE '%{$keyword}%' ) ";
            }
        ...
        $sql = 'SELECT COUNT(id) as count FROM ' .C('DB_PREFIX'). 'lionfish_comshop_package_order ' .$sqlcondition.  $condition ;
        ...
        $sort_way = '`id` DESC';
        if (!(empty($total))) {
            $sql = 'SELECT * FROM ' .C('DB_PREFIX'). 'lionfish_comshop_package_order '  .$sqlcondition . $condition . ' 
					ORDER BY   '.$sort_way.' ';
            if (I('export') != 1) { // 如果导出excel 就显示全部
                $sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
            }
        ...
        if($export == 1){
            ...
            @set_time_limit(0);
            $columns = array(
                array('title' => '订单编号', 'field' => 'order_no', 'width' => 24),
                array('title' => '套餐名称', 'field' => 'goods_name', 'width' => 24),
                array('title' => '套餐数量', 'field' => 'goods_num', 'width' => 12),
                array('title' => '会员姓名', 'field' => 'name', 'width' => 12),
                array('title' => '会员手机号', 'field' => 'mobile', 'width' => 12),
                array('title' => '下单时间', 'field' => 'create_time', 'width' => 24),
                array('title' => '套餐价格', 'field' => 'total_money', 'width' => 12),
                array('title' => '实际支付价格', 'field' => 'pay_money', 'width' => 12),
              	array('title' => '订单状态', 'field' => 'status', 'width' => 12),
            );
            $fileName = date('YmdHis', time());
            D('Seller/Excel')->export($list, array('title' => '套餐订单数据'.$fileName, 'columns' => $columns));
        }
        //dump($sql);die;
    ...
    public function addgoods()
    {
        if (IS_POST) {
            ...
            $list = M('lionfish_comshop_package_goods')->add($data);
    ...
    public function change()
    {
        ...
        M('lionfish_comshop_package_goods')->where( array('id' => $id) )->delete();
    ...
    public function delete()
    {
        ...
        $items = M('lionfish_comshop_package_goods')->field('id,goodsname')->where( array('id' => array('in', $id)) )->select();

        foreach ($items as $item ) {
            //pdo_update('lionfish_comshop_goods', array($type => $value), array('id' => $item['id'])); //ims_lionfish_comshop_goods

            M('lionfish_comshop_package_goods')->where( array('id' => $item['id']) )->delete();
        }
    ...
    function edit(){
        ...
        if (IS_POST) {
           ...
           // $thumb[] =  save_media(resize($item,200,200));
            $data = [
                'goodsname'=>$_GPC['goodsname'],
                'subtitle'=>$_GPC['subtitle'],
                'big_img'=>$_GPC['big_img'],
            ...
            $list = M('lionfish_comshop_package_goods')->where('id = "'.I('get.id').'"')->save($data);
            D('Seller/Goods')->modify_goods();

            $http_refer = S('HTTP_REFERER');

            $http_refer = empty($http_refer) ? $_SERVER['HTTP_REFERER'] : $http_refer;
            $goods = M('lionfish_comshop_package_goods')->where('id = "'.I('get.id').'"')->field('goodsname')->find();
        ...
        S('HTTP_REFERER', $_SERVER['HTTP_REFERER']);
        $this->id = $id;
        $item = M('lionfish_comshop_package_goods')->where('id = "'.I('get.id').'"')->find();
        $this->item = $item;
        $this->display('Packages/addgoods');
    }
