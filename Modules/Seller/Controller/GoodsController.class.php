		...
		if (!(empty($total))) {
			...
			foreach ($list as $key => &$value ) {
                ...
                if(I('get.export') > 0) {// 商品列表到处excel时要导出规格
                    ...
                    $goodsCommon = M('lionfish_comshop_good_common')->where('goods_id = "'.$value['id'].'"')->field('is_spike_buy,is_new_buy,one_limit_count,oneday_limit_count,begin_time,end_time')->find();
...
//        dump($list);die;
	...
	public function addgoods()
	{
		...
		if (IS_POST) {
            ...
            sellerLog('新增商品[' . I('post.goodsname', '', 'trim') . ']', 3);
	...
	**/
	...
	public function change()
	{
		...
		foreach ($items as $item ) {
			...
			//sellerLog('编辑商品[' . $item['goodsname'] . '(' . $item['id'] . ')]{' . $type . ' => ' . $value . '}', 3);
			//记录日志
			if ($type == 'grounding') {
                if ($value == 0 || $value == 1) {
                    sellerLog(($value ? '上架' : '下架') . '商品[' . $item['goodsname'] . '(' . $item['id'] . ')]', 3);
                }
                if ($value == 3) {
                    sellerLog('将商品[' . $item['goodsname'] . '(' . $item['id'] . ')]移到回收站', 3);
                }
            }

            if ($type == 'is_index_show') {
                sellerLog(($value ? '启用' : '禁用') . '商品[' . $item['goodsname'] . '(' . $item['id'] . ')]首页推荐', 3);
            }			
	...
	public function delete()
	{
		...
		foreach ($items as $item ) {
			...
			sellerLog('删除了[' . $item['goodsname'] . '(' . $item['id'] . ')]商品', 3);
		}
	...
	function edit(){
		...
		if (IS_POST) {
			...
			$http_refer = empty($http_refer) ? $_SERVER['HTTP_REFERER'] : $http_refer;
			$goods = M('lionfish_comshop_goods')->where('id = "'.I('get.id').'"')->field('id,goodsname')->find();
			sellerLog('编辑了商品[' . $goods['goodsname'] . '(' . $goods['id'] . ')]', 3);
        ...
        if(!empty($item['jia_user'])){
            $user_id = explode(',',$item['jia_user']);
            foreach ($user_id as $key){
                $jiause[] =  M('lionfish_comshop_jiauser')->where( array('id' => $key ) )->order('id asc')->find();
            }
        }
        $this->jiause = $jiause;
