...
class UserModel{
	...
	**/
	...
	更新会员收益
	*/
	public function changeMemberProfit($member_id,$num, $changetype ,$remark,$charger)
	{
	    $member_info = M('lionfish_comshop_member_commiss')->field('money')->where( array('member_id' => $member_id) )->find();
	    //用户现有的可提现收益
	    $member_profit = $member_info['money'];
	    //
	    //记录数据
	    $flow_data = array();
	    //修改收益数据--start
		...
		if($changetype == 0)
		{
			$up_sql = "update ".C('DB_PREFIX')."lionfish_comshop_member_commiss set money = money+ ".$num ." where member_id=".$member_id;
			$flow_data['remark'] = "收益增加";
		}else if($changetype == 1)
		{
		//减少
			$up_sql = "update ".C('DB_PREFIX')."lionfish_comshop_member_commiss set money = money - ".$num ." where  member_id=".$member_id;
			$flow_data['remark'] = "收益扣除";
		}else if($changetype == 2){
		//最终收益
			$up_sql = "update ".C('DB_PREFIX')."lionfish_comshop_member_commiss set money = ".$num ." where  member_id=".$member_id;
			$flow_data['remark'] = "收益定值";
		}
		//
	    $flow_data['member_id'] = $member_id;
	    $flow_data['detail'] = $remark;
	    //
	    $flow_data['money'] = $num;
	    $flow_data['addtime'] = time();
	    $flow_data['charger'] = $charger;
	    //修改收益数据
		$editResult = M()->execute( $up_sql );
		if($editResult>=0)
		{
		    //添加收益操作信息
		    M('lionfish_comshop_member_profit_flow')->add($flow_data);
		}
	}
	/**
		更新会员积分
