...
(function() {
	var HLS = function(id) {
		...
		this.DOM.logout = { //注销表单
			element: null,
			id: id + '-logout',
			className: this.DOM.className + '-logout'
		};
		this.DOM.wrap = { //播放器默认容器
			element: null,
			id: id + '-wrap',
	...
	HLS.prototype.getVideo = function(request) {
		...
		this.REQUEST.req = this.filter(xbear.extend({}, this.REQUEST.def, this.REQUEST.js, this.REQUEST.data, this.REQUEST.storage, this.REQUEST.url));
		// 处理参数		
		...
		if(pHeight > 50) { //修正需要去掉border,magin,padding
			this.DOM.element.style.overflow = 'auto';
		} else { // 父级未设置高度，需设置默认比例，解决播放窗口异常
			this.CONFIG.ratio = this.CONFIG.ratio || 0.5625;
		}

		if(pChildNum > 1) { //父级有多个子元素
			this.DOM.element.style.height = 'auto';
		} else if(pHeight > 50) { //仅本子元素,且有设置高度
  ...
  // 解析参数
	...
	HLS.prototype.loginRequest = function() {
		...
		request.password = request.password.length == 32 ? request.password : xbear.md5(request.password);
      ...
      channel_id: request.channel_id
		...
		var _this = this;
		xbear.post(this.CONFIG.api, params, function(data) { //post
			try {
			...
			if(data.result == 0) {
				...
				if(_this.CONFIG.login) {
					try {
						sessionStorage.setItem(_this.DOM.id + "-token", _this.REQUEST.token);
					} catch(e) {}
				}
				// 播放列表为空
	...
	HLS.prototype.controlRequest = function(dir, time) {
		...
		var cmd =
			...
			dir === 'right' && [1, 5, 50] ||
			dir === 'zoomin' && [1, 8, 50] ||
			dir === 'zoomout' && [1, 9, 50] || null;
		var request = this.REQUEST.req;
		// 传入参数检查
		if(!this.VIDEO.request.sn || !request.user || !cmd || this.VIDEO.state !== 'onplay') {
			if(!this.VIDEO.control) this.resetMouseEvent();
			return false;
		}
		// 控制器队列加1
		this.VIDEO.control++;
		this.printf(dir + '=' + time + '(' + this.VIDEO.control + ')', 3, '云台控制');
		...
		xbear.post(this.CONFIG.api, params, function(data) { // 可以无需返回
			try {
				data = JSON.parse(data);
			} catch(e) {}
	...
	HLS.prototype.hlsPlayer = function() {
		...
		player.onerror = function() {
			...
			if(_this.VIDEO.player === 'hls-plugin' && player.error.code == 4) {
	...
	HLS.prototype.flashPlayer = function() {
		...
		//处理播放地址
		var ip = this.VIDEO.request.ip || this.REQUEST.req.ip || 'v9.cdn88.cn';
		...
		var flsurl = this.VIDEO.request.flsurl = this.VIDEO.request.flsurl || ip + ':' + port + '||' + user + '||' + sn + '||' + res + '||' + password + '||' + buffer;
		...
		var html = '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0" class="' + this.DOM.player.className + '" id="' + this.DOM.player.id + '" width="100%" height="100%"><param name="movie" value="' + swfPath + '"><param name="quality" value="high"><param name="scale" value="noborder"><param name="bgcolor" value="#ffffff"><param name="allowScriptAccess" value="always"><param name="allowFullScreen" value="true"><param name="FlashVars" value="param=' + flsurl + '"><param name="wmode" value="opaque" /><embed wmode="opaque" src="' + swfPath + '" quality="high" scale="noborder" bgcolor="#ffffff" width="100%" height="100%" allowFullScreen="true" allowScriptAccess="always" FlashVars="param=' + flsurl + '" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object>';
	...
	HLS.prototype.choosePlayer = function(element) {
		if(typeof element.canPlayType !== 'undefined' && element.canPlayType('application/vnd.apple.mpegurl')) {
			return 'hls';
		} else {
			...
			return !!isTypeSupported && !!sourceBufferValidAPI && 'hls-plugin' || 'flash';
		}
	}
	...
	HLS.prototype.reloadPlay = function(time) {
		...
		if(!time || this.VIDEO.timeout || !this.VIDEO.state) {
			return false;
		}
		//定义重载 video.reloadInterval
		this.VIDEO.reloadInterval = setInterval(function() {
			if(_this.VIDEO.player === 'hls' || _this.VIDEO.player === 'hls-plugin') { //hls
				//当前播放位置(秒)
				...
				//异常原因已停止播放
				if(!player.readyState || _this.VIDEO.state !== "onpause" && _this.VIDEO.lastTime === _this.VIDEO.currentTime) {
					_this.printf(_this.VIDEO.player + ":" + _this.VIDEO.state + ",reloading...", 3);
				...
				_this.VIDEO.lastTime = _this.VIDEO.currentTime ? _this.VIDEO.currentTime : null;
			}
			...
			if(_this.CONFIG.timeout && _this.VIDEO.beginTime) {
				...
				// 取整秒,修正误差1秒
				_this.VIDEO.timeout = (_this.VIDEO.endTime - _this.VIDEO.beginTime) / 1000;
				_this.printf("播放持续时间" + ':' + _this.VIDEO.timeout, 3);
				_this.VIDEO.timeout = _this.VIDEO.timeout >= _this.CONFIG.timeout ? _this.VIDEO.timeout - _this.CONFIG.timeout + 1 : 0;
			}
	...
	HLS.prototype.createDom = function() {
		...
		if(this.CONFIG.login && !this.DOM.login.element) {
			html += '<form class="' + this.DOM.login.className + '" id="' + this.DOM.login.id + '">';
			...
			// login user
			html += '<div><label for="' + this.DOM.login.id + '-user">用户名：</label>';
			html += '<input type="text" id="' + this.DOM.login.id + '-user" placeholder="user" value="' + request.user + '"></div>';
			// login password
			html += '<div><label for="' + this.DOM.login.id + '-password">密码：</label>';
			html += '<input type="password" id="' + this.DOM.login.id + '-password" placeholder="password" value="' + request.password + '"></div>';
			// login remember
			var checked = request.remember ? ' checked="checked"' : '';
			html += '<label class="checkbox"><input type="checkbox" id="' + this.DOM.login.id + '-remember"' + checked + '> 记住密码</label>';
			// login submit
			html += '<button type="submit" class="' + this.DOM.className + '-submit">登录</button>';
			html += '</form>';
		}
		// 播放器
		if(!this.DOM.wrap.element) {
			// player Wrap
			html += '<div class="' + this.DOM.wrap.className + '" id="' + this.DOM.wrap.id + '">';
			// player
			html += '<video class="' + this.DOM.player.className + '" id="' + this.DOM.player.id + '" src="" controls autoplay webkit-playsinline playsinline x5-playsinline x-webkit-airplay="allow"></video>';
			// controller
			html += '<div class="' + this.DOM.controller.className + '" id="' + this.DOM.controller.id + '">';
			html += '<div></div><iframe></iframe></div></div>';
		}
		// 播放列表
		if(this.CONFIG.list && !this.DOM.list.element) {
			html += '<ul class="' + this.DOM.list.className + '" id="' + this.DOM.list.id + '"></ul>';
		}
		// 退出登录
		if(this.CONFIG.login && !this.DOM.logout.element) {
			html += '<div class="' + this.DOM.logout.className + '"><button type="button" class="' + this.DOM.className + '-submit" id="' + this.DOM.logout.id + '">注销</button></div>';
		}
		// 日志
		if(this.CONFIG.logs && !this.DOM.logs.element) {
			html += '<textarea class="' + this.DOM.logs.className + '" id="' + this.DOM.logs.id + '"></textarea>';
		}
		// 生成html
		...
		if(_this.CONFIG.login) {
			_this.DOM.login.element.onsubmit = function() {
				...
				request.user = _this.REQUEST.form.user = xbear.trim(document.getElementById(_this.DOM.login.id + '-user').value);
				request.password = _this.REQUEST.form.password = document.getElementById(_this.DOM.login.id + '-password').value;
				request.remember = _this.REQUEST.form.remember = document.getElementById(_this.DOM.login.id + '-remember').checked;

				if(!!request.user) { //登录校验
		...
		// 播放控制事件
		// 鼠标双击全屏/退出全屏
		//xbear.addEvent(_this.DOM.controller.element, 'dblclick', function() {
		//	var wrap = _this.DOM.player.element.parentNode
		//	if(xbear.isFullscreen()) { //退出全屏
		...
		xbear.addEvent(_this.DOM.controller.element, 'mouseup', function() {
			...
			if(_this.VIDEO.mouseEvent.handler && _this.VIDEO.mouseEvent.handler !== 'mouseup') return false;
			// 仅执行一次点击事件
			...
			_this.VIDEO.mouseEvent.timeout = setTimeout(function() {
				...
				if(_this.VIDEO.mouseEvent.times > 1) { //多次点击按双击事件处理(全屏)
					var wrap = _this.DOM.player.element.parentNode;
					if(xbear.isFullscreen()) { //退出全屏
						xbear.exitFullscreen();
					} else {
					...
					_this.VIDEO.mouseEvent.y -= event.clientY;
					// 如果坐标位移小于参考值,判定为点击事件
					if(Math.abs(_this.VIDEO.mouseEvent.x) < reference && Math.abs(_this.VIDEO.mouseEvent.y) < reference) {
						if(_this.VIDEO.state === 'onplay') { //播放
						...
						} else if(_this.VIDEO.state === 'onpause') { //暂停
							if(_this.VIDEO.player === 'flash') {
								_this.VIDEO.state = 'onplay';
								_this.DOM.controller.element.firstChild.className = 'onplay';
						...
						} else if(_this.VIDEO.state === 'onload' && _this.VIDEO.player !== 'flash') { //加载中
							_this.DOM.player.element.load();
						}
						// 重置鼠标事件
						_this.resetMouseEvent();
					} else if(_this.CONFIG.controller && _this.VIDEO.state === 'onplay') { // 云台控制事件
						// 左右控制
						var dir = _this.VIDEO.mouseEvent.x >= reference && 'left' ||
							Math.abs(_this.VIDEO.mouseEvent.x) >= reference && 'right' || '';
						var time = dir && Math.abs(_this.VIDEO.mouseEvent.x) * multiple || 0;
						if(time) { // 有左右控制云台操作
							_this.controlRequest(dir, time);
						}
						// 上下控制
						dir = _this.VIDEO.mouseEvent.y >= reference && 'up' ||
							Math.abs(_this.VIDEO.mouseEvent.y) >= reference && 'down' || '';
						if(dir) { // 存在上下控制
							if(time) { // 如果有左右控制指令则等待
								setTimeout(function() {
									_this.controlRequest(dir, Math.abs(_this.VIDEO.mouseEvent.y) * multiple);
								}, time - reference)
							} else {
								_this.controlRequest(dir, Math.abs(_this.VIDEO.mouseEvent.y) * multiple);
							}
		...
		if(_this.CONFIG.controller) {
			xbear.addEvent(_this.DOM.controller.element, 'mousewheel', function() {
				...
				if(_this.VIDEO.state !== 'onplay' || _this.VIDEO.mouseEvent.handler && _this.VIDEO.mouseEvent.handler !== 'mousewheel') return false;
				// 仅执行一次滚动事件
	...
	HLS.prototype.createListDom = function(devList) {
		...
		for(var i = 0; i < devList.length; i++) {
			var state = devList[i]["state"];
			state = !state && 'online' || 'offline';
			html += '<li class="' + this.DOM.list.className + '-item" data-id="' + i + '">';
			html += '<h3>' + devList[i]["name"] + '</h3>';
			html += '<div class="' + this.DOM.wrap.className + '" style="background-image:url(' + devList[i]["imgsrc"] + ')"><i class="' + state + '"></i>';
			html += '</div></li>';
		}
		// 生成列表
		...
		if(!this.CONFIG.colNum) {
			...
			//需要扩展下事件监听不遍历
			xbear.addEvent(window, 'resize', function() {
				var colNum = Math.floor(_this.DOM.element.clientWidth / _this.CONFIG.colWidth) || 1;
				if(_this.VIDEO.colNum !== colNum) {
		...
		xbear.addEvent(this.DOM.list.element, 'mouseup', function() {
			...
			var nodeName = '';
			while(!!target) { //冒泡到需要执行的事件对象上
				nodeName = target.nodeName.toLowerCase();
				//_this.printf(nodeName);
				if(nodeName === 'video' || nodeName === 'embed' || nodeName === 'object') {
					setTimeout(function() { //hls-plugin会连续播放暂停，需要延后判断
			...
			if(nodeName === 'li') {
				...
				if(_this.VIDEO.request.state) {
					_this.stopPlay();
					_this.printf("设备不在线", 7);
					return false;
				}
				// 播放选择标记 //div
	...
	HLS.prototype.implodeDev = function(devObj) {
		var dev = new Array();
		var type = typeof devObj === 'object' && xbear.type(devObj) || typeof devObj;
		//json字符串尝试先转换成数组或对象
		if(type === 'string') {
	...
	HLS.prototype.printf = function(content, type, prefix) {
		type = type || 1;
		prefix = prefix || '';
		if(typeof content === 'undefined') { //无内容或未指定日志类型时退出
			return false;
		}
		//将对象转换为文本,并清除多余"\"
		var str = typeof content === 'object' ? JSON.stringify(content).replace(/\\/g, "") : content;
		//增加前缀
		str = prefix ? prefix + " : " + str : str;

		// 1 debbug + type in(1,3,5,7)
		if(this.CONFIG.debug && (type == 1 || type == 3 || type == 5 || type == 7)) {
		...
		if(this.CONFIG.logs && (type == 2 || type == 3 || type == 6 || type == 7)) {
		...
		if(this.CONFIG.alert && (type == 4 || type == 5 || type == 6 || type == 7)) {
			alert(str);
		}
...
var xbear = {
	...
	utf8: {
		encode: function(str) {
			...
			var strUtf = str.replace(/[\u0080-\u07ff]/g, function(c) {
		...
		decode: function(str) {
			var c = String.fromCharCode(0);
			var strUni = str.replace(/[\u00e0-\u00ef][\u0080-\u00bf][\u0080-\u00bf]/g, function(c) {
